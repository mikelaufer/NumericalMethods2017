...
advection_velocity = np.array([0.0, 0.0, 0.0])
...
while not converged:
    ...
    # add terms for the advection part
    for elem_index in range(msh.nelements):
        diag = 0.0
        for tmp_index in range(msh.elem_nfaces):
            nbr_index  = msh.neighbors[elem_index, tmp_index]
            if nbr_index != -1:
                ...
                Gf    = advection_velocity.dot(msh.element_normals[elem_index, tmp_index])*ds
                diag += 0.5*(Gf + abs(Gf))

                A[elem_index, nbr_index] += -0.5*(Gf - abs(Gf))

        A[elem_index, elem_index] += diag
